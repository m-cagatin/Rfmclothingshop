...
export class SavedDesignsService {
   ...
   */
  static async saveToLibrary(data: SaveToLibraryData): Promise<SavedDesignData> {
    const {
      userId,
    ...
    const design = await prisma.user_saved_designs.create({
      data: {
        user_id: userId,
   ...
   */
  static async getAllSavedDesigns(userId: string): Promise<any[]> {
    const designs = await prisma.user_saved_designs.findMany({
      where: {
        user_id: userId
      },
      include: {
   ...
   */
  static async deleteSavedDesign(designId: number, userId: string): Promise<boolean> {
    try {
      // Verify ownership before deleting
      const design = await prisma.user_saved_designs.findFirst({
        where: {
          id: designId,
          user_id: userId
        }
      ...
      await prisma.user_saved_designs.delete({
        where: {
          id: designId
        }
      });
   ...
   */
  static async getSavedDesignById(designId: number, userId: string): Promise<SavedDesignData | null> {
    const design = await prisma.user_saved_designs.findFirst({
      where: {
        id: designId,
        user_id: userId
