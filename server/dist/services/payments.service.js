...
const cashflowService = __importStar(require("./cashflow.service"));
    ...
    if (!order) {
        ...
        const orderItemsData = await Promise.all(orderItems.map(async (item) => {
            ...
            const customizationData = item.customizationData
                ? JSON.parse(JSON.stringify(item.customizationData))
 ...
 * Approve a payment
 */
async function approvePayment(paymentId, verifiedBy) {
    const payment = await prisma.payments.findUnique({
        where: { payment_id: paymentId },
        include: {
 ...
 * Reject a payment
 */
async function rejectPayment(paymentId, verifiedBy) {
    const payment = await prisma.payments.findUnique({
        where: { payment_id: paymentId },
    });
    if (!payment) {
